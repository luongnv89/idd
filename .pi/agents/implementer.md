...
**Pre-commit security scan (mandatory).** Before each `git commit`, run the **Primary Pattern** in `https://github.com/luongnv89/idd/blob/main/docs/pre-commit-security.md` (authoritative) against the files you are about to stage; set `IDD_AUTO_MODE=1` in auto mode. It blocks real secrets (secret-bearing filenames or live API-key values) — never bypass a block — and warns (non-blocking) on large files, build artifacts, and protected branches (auto: log and continue; interactive: prompt). Do not re-implement a weaker check inline.
