...
- **Inputs:** `{branch_name}`, `{base_branch}`, `{issue_context}`, `{pr_context}`, `{findings_json}` (fixable findings from reviewer/AC/traceability/test/CI), `{test_output}` (trimmed), `{commit_message}`, `{security_convention}` (path to the pre-commit security scan document — mandatory before committing), `{secscan_script}` (path to the bundled script that implements it; empty when the orchestrator ships none), `{secscan_policy_ref}` (the ref the scan must read `security.*` from — a ref the branch under review cannot write; empty when the orchestrator supplies none), and optional `{workspace_contract}` (`lane_id`, canonical absolute `repo_root` / `worktree_path`, branch, full base SHA) plus independently supplied `{expected_lane_identity}` (`lane_id`, issue, branch, worktree path).
...
7. Commit if files changed: stage specific files only (`git add <file>`, never `.`). Before committing, run the mandatory pre-commit security scan against the staged set — real secrets MUST block (stop, report FAILED with the path); warnings follow auto (IDD_AUTO_MODE=1: log+continue) vs interactive (stop+surface). Prefer the script when {secscan_script} is set: `python3 {secscan_script} --staged`, run from the repo root — it reads the repo's security.* extensions from .gitissue.yml itself, so pass no configuration on the command line. When {secscan_policy_ref} is non-empty, append `--policy-ref "{secscan_policy_ref}"` — never with an empty value, which the script rejects as a usage error: .gitissue.yml is repo-controlled, and a branch whose own security.allow_pattern governs its own scan can allow-list the file carrying the secret. Exit 1 is the block verdict — stop and report FAILED with the path from blocking[]; do NOT re-run the prose scan hoping for a pass. Exit 3 is also a stop. Exit 0 is a pass only when the JSON also shows verdict not "block", policy_source equal to ref: plus the ref you passed, whenever you passed one (the prefix is part of the reported value: ref:origin/main, never bare origin/main), and NOT (scanned 0 while skipped is above 0) — a scan that examined nothing has not passed. Report a scanned/skipped/policy_source mismatch as FAILED, never as a clean scan. A missing python3, exit 2 (the path did not resolve — your working directory is the target repo, not the skill directory), or exit 4 all degrade to the Primary Pattern in {security_convention}, which is also what you run when {secscan_script} is empty. Never treat a scan that did not run as a scan that passed. Commit with {commit_message}. Never push.
