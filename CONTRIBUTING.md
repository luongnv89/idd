...
When you change anything under `src/`, rerun the build before committing — CI's drift check (`.github/workflows/dist-check.yml`) compares the committed `skills/` against a transient build on every PR and fails if they differ. Skip the rebuild and your PR will fail.

The build is byte-deterministic: rerunning it on an unchanged `src/` produces an identical `skills/`.
