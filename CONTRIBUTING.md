...
`src/` is the single source of truth. `skills/` is the committed install surface, and `dist/` is generated by `./scripts/build.sh` (plugin tarball only, gitignored).

```bash
./scripts/build.sh          # Rebuild dist/plugin/ from src/ (skills/ is already committed)
```
