...
└── plugin.json.in          # Plugin manifest template (rendered at build time)

dist/                       # GENERATED — do not hand-edit
├── skills/<name>/          # Committed: standalone install path
└── plugin/                 # Gitignored: built fresh, shipped as release tarball

scripts/
├── build.sh                # Thin entrypoint → calls build.py
└── build.py                # Deterministic builder (src/ → dist/)
...
`src/` is the single source of truth. `dist/` is generated by `./scripts/build.sh` and must never be hand-edited.

```bash
./scripts/build.sh          # Rebuild dist/skills/ and dist/plugin/ from src/
```

When you change anything under `src/`, rerun the build before committing — CI's drift check (`.github/workflows/dist-check.yml`) compares the committed `dist/skills/` against a fresh build on every PR and fails if they differ. Skip the rebuild and your PR will fail.

The build is byte-deterministic: rerunning it on an unchanged `src/` produces an identical `dist/`.
