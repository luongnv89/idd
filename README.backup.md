...
<p align="center">
  <a href="https://github.com/luongnv89/idd/releases/latest"><img src="https://img.shields.io/badge/version-0.1.0-blue.svg" alt="Version 0.1.0"></a>
...
| `/issue-analysis N` | Deep-analyzes a single issue: root cause, git history, cross-references, 2-3 implementation options, complexity and risk |
...
graph TD
    ...
    D --> E["/issue-analysis N (optional, deep dive)"]
    E --> F["/issue-resolver N"]
    F --> G["Fetch → Branch → Research → Plan → Execute → Verify → Ship"]
    G --> H[Atomic PR linked to issue]
    H --> I["Review → Merge → Issue auto-closes"]
...
```
[1/8] Fetch          — Load issue, classify type (bug/feature/improvement)
[2/8] Extract        — Parse keywords, file refs, error messages from issue body
[3/8] Research       — Deep codebase scan (30 files, 3-level import tracing)
[4/8] History        — Git log analysis: prior fix attempts, regressions, domain experts
[5/8] Cross-refs     — Triage data + related issues/PRs: duplicates, already resolved
[6/8] Analysis       — Root cause (bugs), architecture fit (features), current impl (improvements)
[7/8] Options        — 2-3 implementation approaches with pros/cons/complexity/risk
[8/8] Report         — Full terminal report + persist to .gitissue/analysis-N.json
```

Use `/issue-analysis N view` to render a cached analysis without re-scanning.
...
```
skills/
├── issue-analysis/     # /issue-analysis N — deep single-issue analysis
│   ├── SKILL.md
