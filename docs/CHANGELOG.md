...
- `fix(issue-pr-review)` Restored the bundled-dependency precheck's runtime-doc coverage. The 2.3.0 refactor rewrote the precheck from an explicit file list to a pointer ("the `references/docs/*.md` enumerated in *Additional Resources*"), but the *Additional Resources* section listed only `naming-conventions.md` — dropping `pre-commit-security.md` and `sync-conventions.md` from the upfront existence guard even though the skill body still depends on both at runtime (pre-commit-security.md 4×, sync-conventions.md 1×). On a broken/partial install the skill would sail past the precheck and fail mid-run instead of failing fast with the clean-reinstall message, and the dropped doc was the security-scan contract the precheck text says must never be skipped. Added both docs back to *Additional Resources* so the pointer resolves to the full set. No behavior change on a clean install. Skill bumped: `/issue-pr-review` 2.3.0 → 2.3.1.
