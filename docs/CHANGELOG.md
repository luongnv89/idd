...
### Added
- `feat(monitoring)` Added `.gitissue/runs.jsonl` — an **append-only run-log** that gives the IDD *monitoring* value its first persistent home (closes W3). `/issue-resolver` appends one JSON line per run on **every terminal path** (successful PR → `left_open`; already-fixed early-exit, closed issue, or existing PR → `skipped` with a `skipped_reason`; aborted pipeline → `failed`), and `/auto-pilot` appends one enriched line per processed issue (including skips). To avoid double-logging when auto-pilot drives the resolver, the resolver accepts a `--no-run-log` flag (passed only by auto-pilot) and returns its telemetry (`complexity`, `qa_cycles`, `duration_s`, outcome) so the orchestrator writes the single line. The file is schema-light, grep-friendly, deletable like the rest of `.gitissue/`, and **never staged** into the resolution PR (written after PR creation; skills never `git add` it). `/idd-doctor` gains an informational **run summary** — resolve rate, median QA cycles, outcome breakdown, and common skip reasons over the last 50 runs — that degrades gracefully when the file is absent or has malformed lines and never affects its PASS/WARN/FAIL result. Documented in `docs/config-schema.md`. Skills bumped: `/issue-resolver` 0.10.0 → 0.11.0, `/auto-pilot` 2.3.2 → 2.4.0, `/idd-doctor` 0.1.0 → 0.2.0. (#141)
