...
Internal-only skills live in `src/internal-skills/` (e.g., `idd-doctor`); deprecated skills retained for one release cycle live in `src/deprecated-skills/`. Runtime docs consumed by skills live in `src/docs/`. Top-level `docs/` retains human-only project documentation (architecture, changelog, development guide).
...
`/issue-pr-review` enforces this dual-write rule via presence-only and per-criterion verification gates (issues #35 and #36). See [`src/docs/idd-methodology.md`](../src/docs/idd-methodology.md) for the full rationale.

## Configuration

`.gitissue.yml` is loaded once at skill start. The full schema is documented in [`src/docs/config-schema.md`](../src/docs/config-schema.md).
...
|-----------|----------|---------|
| GitHub Projects sync | `src/docs/github-projects-sync.md` | issue-creator, issue-resolver, issue-triage |
| Naming conventions | `src/docs/naming-conventions.md` | issue-creator, issue-resolver, auto-pilot |
| IDD methodology + analysis-artifact rule | `src/docs/idd-methodology.md` | issue-resolver, issue-pr-review, issue-analysis |
| Configuration schema (autopilot + review) | `src/docs/config-schema.md` | auto-pilot, issue-pr-review, init-gitissue |

### GitHub Projects Sync

A shared utility reference that skills invoke to update issue status on a repo's GitHub Project board. It wraps the GitHub Projects v2 GraphQL API (`gh api graphql`) to discover linked projects, add issues, and update status fields (Todo, In Progress, Done). Controlled by the `projects` section in `.gitissue.yml`. Gracefully degrades when no project board exists or the API fails — skills continue without blocking. See `src/docs/github-projects-sync.md` for the full procedure reference.
