...
The `.gitissue/` directory stores persistent state generated by skills (`docs/config-schema.md:582-587`):
...
| `runs.jsonl` | `/issue-resolver`, `/auto-pilot` | Append-only run log, one line per processed issue (`src/shared/scripts/gi-runlog.py:2`) |
...
| `cache/` | `/issue-resolver` and other `gi-issue.py` callers | TTL-cached `gh issue view` payloads (`src/shared/scripts/gi-issue.py:8`) |
...
| `gi-gh.py` | Shared subprocess boundary for GitHub CLI calls (`src/shared/scripts/gi-gh.py:2`) | issue-analysis, issue-creator, issue-pr-review, issue-resolver, auto-pilot |
...
6. **Durable memory in git** — every resolution dual-writes its Decision Record and AC verification into both the PR body and (via squash merge, where `squash_merge_commit_message` is `PR_BODY` — see *Analysis Artifacts and Durable Memory*, item 2) the commit body, so `git log -p` is a self-contained project history independent of `.gitissue/` files
