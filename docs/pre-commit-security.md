...
- **Binary skip.** Real-key scanning skips files whose MIME charset is `binary`. Without this, scanning a large image or PDF burns time and can false-positive on encoded bytes. **Residual risk (decided in #276 — kept deliberately):** `gi-secscan.py` implements this as a NUL-byte sniff over the first 8 KiB (`_BINARY_SNIFF`), a fast proxy for the `charset=binary` rule rather than an exact match — e.g. UTF-16LE text carrying a byte-order mark reports `charset=utf-16le` (not binary) yet still carries NUL bytes inside the sniff window (every ASCII-range character's high byte), so the two paths can diverge; this particular case is benign, since the ASCII-anchored real-key pattern cannot match interleaved-NUL content through either path. The sharper risk is structural: any file engineered to carry a NUL byte in its first 8 KiB bypasses value-scanning entirely — one member of an unbounded evasion family (base64, gzip, split-across-lines, or padding past the sniff window) whose full closure would mean content-scanning every binary file on every commit, the cost this rule exists to avoid. Filename- and size-based rules still fire regardless of this skip. The trade-off matches industry practice — gitleaks skips binary content in its git-scanning mode too, and its own tracking issue for the same gap (gitleaks#1541, "Scan binary files") was closed as not planned — a maintainer team actively weighing and rejecting binary-content scanning is stronger precedent for keeping this skip than an unresolved backlog item would be — and is accepted here deliberately, not an oversight; see #276 for the full reasoning.
...
5. **Take the policy from the reviewing side, and check what the scan examined.** `.gitissue.yml` is repository-controlled, and `security.allow_pattern` suppresses **scanning**, not findings — a path it matches never reaches a rule. A skill that scans a branch it does not control (`/issue-pr-review` reviews the branch that wrote the checked-out config; `/issue-resolver` pushes a branch built from an untrusted issue body) MUST therefore read `security.*` from a ref that branch cannot write — `--policy-ref origin/<default-branch>`, bound from the repository's default branch and never from a PR's author-chosen base — and MUST treat exit `0` as a pass only when the verdict also reports `policy_source` equal to that ref carrying its `ref:` prefix (`ref:origin/main`, not bare `origin/main`), `verdict` other than `block`, and **not** (`scanned` of 0 while `skipped` is above 0). A scan that examined nothing is not a clean scan. `scanned > 0` alone is not the test: a *narrow* allow pattern naming only the secret-bearing file leaves `scanned` healthy, and only the policy's provenance rules it out. A skip authorized by the trusted ref is the allow-list feature working correctly, so `skipped` above 0 is not by itself a failure.
...
| **Within 12 lines above** | An invocation **only** — in prose *or* in a preceding **shell** block. Two adjacent shell blocks, "scan" then "push", is a gated pattern. | A citation of this document. Every `SKILL.source.md` ends with an "Additional Resources" index naming `pre-commit-security.md`; accepting it from above would auto-gate anything appended near the end of a file by proximity to a table of contents. Also: an invocation in a **display-only** fence (see below). |

Five rules decide whether a line is an invocation at all:
...
- **Display-only fences do not gate from above.** The same info string that decides whether a block is a *subject* decides whether it can be a *gate*. A ` ```text ` transcript of the scan running, a ` ```markdown ` template quoting the command, a ` ```json ` payload carrying it as a string, or a bare ` ``` ` listing is display-only, and a transcript of the scan is not the scan. Only prose and shell fences reach the block below them.

What this does **not** separate: inside a shell fence the invocation gates whether or not it is one you meant to run. A ` ```bash ` block written as a counter-example, or one whose heredoc merely *writes* the command into a file, still gates the block after it — the fence is the only signal available, and telling those apart needs a shell parser. Write counter-examples in a display-only fence, or move them out of the 12-line window.

Two non-vacuity guards sit under the scan, because "all subjects are gated" is also true of zero subjects. The fence parser handles indented fences, backtick and tilde fences of any length, info strings (```` ```bash title="x" ````), and a fence left unclosed at end of file — which CommonMark renders as an ordinary code block, so an ungated `git push` in the last fence of a file would otherwise never be looked at. Indentation is bounded the way CommonMark bounds it, *relative to the enclosing fence* rather than to column zero: an opening fence is accepted at any indent, because a fence nested in a `10. ` list item starts at column 4 while being indented 0 within its own container, and a closing fence is accepted only within 3 columns of its opener, because a deeper run is block content and closing on it would invert fence parity for the rest of the file. An absolute bound breaks both halves at once — it hides every list-nested block from the scan, and it lets a scan transcript inside a list-nested display-only fence leak into the prose window and gate a push it has nothing to do with.

Underneath the indentation rules the parser guards one failure directly, because every defeat it has suffered has been the same one: a block the parser believes is open swallowing a real ` ```bash ` block whole. That phantom is reachable three ways — a false close (a run indented past its opener), a false open (a top-level indented code block that happens to contain a ``` run), and a fence the author simply never closed. Only the first has an entrance guard: the closing-fence indent bound above. The other two have none, and are caught only at the exit — when the open block is **display-only**, a same-character fence run of length ≥ the opener's carrying a non-empty info string is taken as a sign the open block was a phantom, so it is evaluated and closed and the real block opens from that fence. The check runs *before* the indentation bound, since that bound reasons about content of an enclosing block that in this case does not exist.

That recovery is a heuristic, and the restriction to display-only blocks bounds it rather than making it free. CommonMark does not forbid the shape inside an open fence — every line in a fenced block is literal content, and only an empty-info run closes one — so the shape is evidence, not proof. Nor does the recovery merely abandon the open block: it calls the same open-block path on that fence, so a genuine three-backtick ` ```text ` wrapper quoting a nested ` ```bash ` sample has the sample promoted to a real shell block, and a `git push` inside the sample is reported as a violation. (The ` ````markdown ` wrapper idiom escapes that only because its opener is longer, so the branch never fires; a three-backtick display wrapper has no such exemption.) What justifies the restriction is the asymmetry of the two misfires: on a display-only block it is a false positive the author sees and can rewrite, while the same guess inside a real ` ```bash ` block is silent — a heredoc that writes a fenced snippet would end the block before its `git push` is reached, and the push would become neither subject nor violation. Measured over the whole of `src/` the branch fires zero times, so no block changes hands either way today.

The class is narrowed rather than closed, and three residues are accepted knowingly. Because the recovery keys on a matching fence character and length, it does not fire when the phantom is a ` ~~~text ` fence or a 4-backtick ` ````text ` fence; because it keys on the open block being display-only, it does not fire when the phantom is one the parser reads as a **shell** block — an unclosed ` ```bash `/` ```console ` fence, or an indented code block containing a ` ```bash ` run. Each still swallows a following ` ```bash ` block. The first two are latent: `src/` contains zero tilde fences and zero 4-backtick fences. **The third is live** — `src/` carries 96 bash-class fences, so one missing closer reaches it — and it is the worst of the three, because the merged block is itself a shell block: a swallowed `git push` is recorded as a subject at the *phantom's* line and shares the merged block's single gate flag, so an invocation anywhere in the merged span reads as gating it. That residue predates this guard rather than being introduced by it, and closing it is tracked separately. Widening the character/length match would mean recovering across exactly where the legitimate ` ````markdown ` wrapper lives, so that trade buys two shapes nothing here writes at the cost of a false positive against one written often; the shell-block residue needs a different fix, not a wider match. If a tilde or 4-backtick fence appears in `src/`, revisit this rather than assuming it is handled.

And the test pins the exact list of files the subjects live in — not merely how many there are, which any two blocks would satisfy. When a skill legitimately gains or loses a `git commit`/`git push` block, update `EXPECTED_SUBJECT_FILES` in `tests/test-pre-commit-security.sh` in the same commit.
