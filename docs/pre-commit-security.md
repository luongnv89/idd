...
Underneath the indentation rules the parser guards one failure directly, because every defeat it has suffered has been the same one: a block the parser believes is open swallowing a real ` ```bash ` block whole. That phantom is reachable three ways — a false close (a run indented past its opener), a false open (a top-level indented code block that happens to contain a ``` run), and a fence the author simply never closed — and a swallowed block is invisible to every other guard here, since the push never becomes a subject and the pinned file list is therefore unchanged. So besides blocking each entrance, the parser also recovers at the exit: when the open block is **display-only**, a same-character fence run of length ≥ the opener's carrying a non-empty info string is taken as a sign the open block was a phantom, so it is evaluated and closed and the real block opens from that fence. The check runs *before* the indentation bound, since that bound reasons about content of an enclosing block that in this case does not exist.

That recovery is a heuristic, and the restriction to display-only blocks is what makes it safe rather than incidental. CommonMark does not forbid the shape inside an open fence — every line in a fenced block is literal content, and only an empty-info run closes one — so the shape is evidence, not proof. Abandoning a display-only block early costs nothing, because subjects only live in shell blocks; applying the same guess inside a real ` ```bash ` block is destructive, since a heredoc that writes a fenced snippet would end the block before its `git push` is reached and the push would become neither subject nor violation. The ` ````markdown `-wrapping-` ```bash ` idiom is unaffected for a separate reason: there the opener is longer, so the branch never fires.

The class is narrowed rather than closed, and two residues are accepted knowingly. Because the recovery keys on a matching fence character and length, it does not fire when the phantom is a ` ~~~text ` fence or a 4-backtick ` ````text ` fence, and either still swallows a following ` ```bash ` block. Both are latent, not live: `src/` contains zero tilde fences and zero 4-backtick fences. Widening the match would mean recovering across fence characters and lengths, which is exactly where the legitimate ` ````markdown ` wrapper lives, so the trade would buy two shapes nothing here writes at the cost of a false positive against one that is written often. If either shape appears in `src/`, revisit this rather than assuming it is handled.
