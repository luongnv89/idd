...
| **`/auto-pilot`** | **3330** (lower bound) — 3670 upper; **3046** on the lower bound with every single-word category grep discounted | Granted on **D3** and corroborated by the floor. The compression pass ran in #415 / PR #433: 6026 → 5216 `wc -w`, 481 → 428 lines, 5941 → 5129 `asm` words, and the floor was re-measured **after** it by the method above (30 tracked suites traced under `bash -x`, 110 live patterns replayed and unioned) — it fell 171 words and stayed over. Deleting every free-prose word still leaves 3330. Its pinned material is the per-phase gates, the merge-mode table, the stop-condition table, the `## Configuration` block and the run-log write contract; all are raw `grep`s naming `SKILL.source.md`, which the class table forbids re-targeting, and all are every-run gate logic that D3 forbids relocating in any case. The remaining free prose is walked block by block in `docs/experiments/skill-auto-improver-auto-pilot.md` §6.2: none of it is conditional at run time, and one block (*Dependency Preflight*) is required by Gate 1 itself. |
