...
Concretely, `/issue-resolver` lifts a five-field **Decision Record** — root cause, options considered, options rejected, selected option, residual risk — out of the analysis JSON and into the PR body, alongside an acceptance-criteria verification table. For **bug** issues it carries a sixth field, the **reproduction** evidence (the command confirmed red for the stated reason before the fix, and the regression test proving the red → green transition), produced by the bug-verification checkpoint `/issue-resolver` runs before applying a fix and likewise dual-written. `/issue-pr-review` checks for the presence of those fields as part of its traceability dimension. Deleting `.gitissue/analysis-<N>.json` after the PR merges then costs nothing — the reasoning is already in `git log -p`, *provided the git-history binding below actually landed*.

The PR body carries one more durable field: a **QA handoff marker** — `<!-- gitissue:qa v1 head=<sha40> … -->` on its last line, written by `/issue-resolver` only on a clean QA exit — recording what it already verified, pinned to the head commit, so `/issue-pr-review` collapses work it would otherwise repeat into one confirmation pass. The pin self-invalidates on any later commit yet survives a body edit: the binding is on code, not prose. It is deliberately **not** authentication — a PR body is author-editable — so it may gate duplicated work only; secret scanning, CI, acceptance-criteria verification and traceability always run at full strength.

The git-history half of the dual write is satisfied by one of three bindings ([SPEC.md §4.3](https://github.com/luongnv89/idd/blob/main/SPEC.md)): the **squash-merge body** (the platform copies the PR body into the squash commit, *when the repo is configured to do so*), a **merge-commit body** (the record is appended to the merge commit message), or **git notes** (the record is attached to the landing commit under a documented notes ref). The reference implementation assumes **squash-merge as the default binding** — it is the only one that needs zero extra tooling on GitHub.

That assumption is **conditional on a repo setting, not platform behavior** (issue #295): the squash commit carries the PR body only where `squash_merge_commit_message` is `PR_BODY`. GitHub's default, `COMMIT_MESSAGES`, writes the commit subjects — the strategy *is* squash and the record is dropped anyway. Read it, never assume it: `gh api repos/{owner}/{repo} --jq .squash_merge_commit_message`; `gh repo view --json` cannot report it, which is why a strategy check passes while the binding is defeated. Repos on another strategy, or another message source, keep durable memory only on the tracker unless they declare another binding. So `/init-gitissue` warns on the strategy **and** the `squash_merge_commit_*` sub-settings, and `/issue-pr-review` reports a non-`pass` traceability status instead of asserting it. Commits already landed under a defeated binding are **not** backfilled: [ADR: no backfill](https://github.com/luongnv89/idd/blob/main/docs/decisions/no-backfill-merged-decision-records.md).
