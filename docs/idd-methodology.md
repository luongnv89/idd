...
`/issue-analysis` writes its findings to `.gitissue/analysis-<N>.json` so the resolver can reuse them and reviewers can inspect the current-code reality it ran against. Reuse is conditional on the artifact still being true: `/issue-resolver` treats it as a research baseline — seeding a verify-first pass and skipping synthesis — only when the predicate whose single home is its *Step 0h — Analysis reuse gate* holds; any doubt falls back to the full pipeline. That JSON is **local cache**: safe to delete, regenerated by a fresh `/issue-analysis N`. It cannot be the home of project memory — once the PR merges and the cache is cleared, the reasoning goes with it.

Durable project memory therefore lives in artifacts that already survive: the **issue body** (intent), the **PR body** (analysis lift, decision record, acceptance verification), and a **git-history artifact** on the default branch carrying the same content. The **dual-write rule**: any analysis signal worth preserving must appear in both the PR body and git history.

Concretely, `/issue-resolver` lifts a five-field **Decision Record** — root cause, options considered, options rejected, selected option, residual risk — out of the analysis JSON and into the PR body, alongside an acceptance-criteria verification table. For **bug** issues it carries a sixth field, the **reproduction** evidence (the command confirmed red before the fix, and the regression test proving red → green), produced by the bug-verification checkpoint and likewise dual-written. `/issue-pr-review` checks those fields as part of its traceability dimension. Deleting `.gitissue/analysis-<N>.json` after the PR merges then costs nothing — the reasoning is already in `git log -p`, *provided the git-history binding below actually landed*.

The PR body carries one more durable field: a **QA handoff marker** — `<!-- gitissue:qa v1 head=<sha40> … -->` on its last line, written by `/issue-resolver` only on a clean QA exit — recording what it verified, pinned to the head commit, so `/issue-pr-review` collapses work it would otherwise repeat into one confirmation pass. The pin self-invalidates on any later commit yet survives a body edit: the binding is on code, not prose. It is deliberately **not** authentication — a PR body is author-editable — so it gates duplicated work only; secret scanning, CI, acceptance-criteria verification and traceability always run at full strength.

The git-history half of the dual write is satisfied by one of three bindings ([SPEC.md §4.3](https://github.com/luongnv89/idd/blob/main/SPEC.md)): the **squash-merge body** (PR body copied into the squash commit), a **merge-commit body** (record appended to the merge commit message), or **git notes** (record attached to the landing commit under a documented notes ref). The reference implementation assumes **squash-merge** — the only one needing zero extra tooling on GitHub — but that is **conditional on a repo setting, not platform behavior** (issue #295): the squash commit carries the PR body only where `squash_merge_commit_message` is `PR_BODY`. GitHub's default, `COMMIT_MESSAGES`, writes the commit subjects instead, so the strategy *is* squash and the record still dropped — which is why a strategy-only check passes while the binding is defeated. Read it, never assume it: `gh api repos/{owner}/{repo} --jq .squash_merge_commit_message`. Repos on another strategy or message source keep durable memory on the tracker only. `/init-gitissue` warns on the strategy **and** the `squash_merge_commit_*` sub-settings; `/issue-pr-review` reports a non-`pass` traceability status rather than asserting one. Commits landed under a defeated binding are **not** backfilled: [ADR: no backfill](https://github.com/luongnv89/idd/blob/main/docs/decisions/no-backfill-merged-decision-records.md).
