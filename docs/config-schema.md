...
| `.gitissue/runs.jsonl` | `/issue-resolver`, `/auto-pilot` | Append-only run log — one JSON line per resolve run / per processed auto-pilot issue. Cross-run telemetry for monitoring (see below). |
...
summary, so truncation or deletion only resets the telemetry window.
