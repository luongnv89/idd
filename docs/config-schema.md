...
autopilot:
  ...
  # convention. See docs/idd-methodology.md (Issue Dependencies) for the marker
...
| `.gitissue/runs.jsonl` | `/issue-resolver`, `/auto-pilot` | Append-only run-log — one JSON line per resolve/auto-pilot run. Cross-run telemetry summarized by `/idd-doctor`. See [Run-log schema](#run-log-runsjsonl) below |
...
- Skills **write** their artifacts into the working tree but never `git add`/`git commit` them as part of a run — the files are tracked because they were committed once, not re-committed on every invocation. This keeps `runs.jsonl` (and the other artifacts) out of unrelated feature-branch diffs.

### Run-log (`runs.jsonl`)

`.gitissue/runs.jsonl` is an **append-only, schema-light, grep-friendly** log of every `/issue-resolver` and `/auto-pilot` run. It gives the IDD *monitoring* principle its first persistent home — without a dashboard or external service — so a maintainer can answer questions like "how often does QA need 3+ cycles?" or "what's the most common auto-pilot skip reason?" without re-reading scrollback. `/idd-doctor` reads it to summarize recent runs.

**Format:** one JSON object per line (JSON Lines / [`.jsonl`](https://jsonlines.org/)). New runs append a single line; existing lines are never rewritten. This makes the file resilient to concurrent appends and trivial to `tail`, `grep`, or pipe through `jq`.

**Fields** (a line includes at least `ts`, `issue`, `mode`, `outcome`; other fields appear when known):

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `ts` | string | ✓ | UTC ISO-8601 timestamp, `date -u +%Y-%m-%dT%H:%M:%SZ` (e.g. `2026-06-26T14:03:21Z`) |
| `issue` | number | ✓ | GitHub issue number the run targeted |
| `mode` | string | ✓ | `"resolve"` (standalone `/issue-resolver`) or `"auto-pilot"` (an `/auto-pilot` iteration) |
| `outcome` | string | ✓ | One of the six categorical outcomes (see below) |
| `pr` | number | — | PR number created/merged for this run; omit when no PR was produced |
| `complexity` | string | — | `"low"` \| `"medium"` \| `"high"` — the researcher's complexity assessment |
| `qa_cycles` | number | — | Number of QA review-fix cycles the resolve pipeline ran |
| `duration_s` | number | — | Wall-clock seconds for the run, when measured |
| `skipped_reason` | string | — | Why the issue was skipped (present only when `outcome` is `skipped`), e.g. `"already_resolved"`, `"blocked_label"`, `"assigned_other"`, `"blocked_by_dependency"`, `"in_skip_list"` |

**`outcome` vocabulary** — reuses the six categorical outcomes already used by `/auto-pilot`'s iteration log so the signal stays consistent across skills:

| Outcome | Meaning |
|---------|---------|
| `merged` | PR created and merged |
| `left_open` | PR created, intentionally left open (e.g. conservative mode) |
| `partial_followup` | PR created with unresolved review issues; a follow-up issue was filed |
| `blocked_by_dependency` | A PR **was** created but cannot merge until an unmet `Depends on #N` / `Blocked by #N` dependency is itself merged (the merge gate blocked it) |
| `failed` | Run failed before producing a mergeable PR (build/test failure, error) |
| `skipped` | Skipped before resolution started — see `skipped_reason` |

> **`blocked_by_dependency` — outcome vs. skip reason.** A dependency block can surface at two different lifecycle moments, and they are recorded differently (this matters because the doctor's resolve-rate counts an `outcome: "skipped"` run as *not attempted* but an `outcome: "blocked_by_dependency"` run as an *attempted, non-merged* one). Use:
> - **`outcome: "blocked_by_dependency"`** when the issue **was picked** and got a PR that the Phase 5 merge gate then refused to merge because a dependency is still open.
> - **`outcome: "skipped"`, `skipped_reason: "blocked_by_dependency"`** when the issue was **never picked** — triage/selection passed it over because a dependency wasn't ready, so no resolution work happened.

Standalone `/issue-resolver` produces only the resolve-stage subset it can observe: `merged` is never emitted by the resolver alone (it does not merge), so a resolve run records `left_open` on a successful PR, `failed` on an aborted pipeline, or `skipped` (with `skipped_reason: "already_resolved"`) on the already-fixed early-exit. The merge-related outcomes (`merged`, `partial_followup`, `blocked_by_dependency`) are written by `/auto-pilot`, which owns the merge decision.

**Example:**

```jsonl
{"ts":"2026-06-26T14:03:21Z","issue":141,"mode":"resolve","complexity":"medium","qa_cycles":2,"outcome":"left_open","pr":153,"duration_s":412}
...
**Conventions:**
- **Append-only.** Skills append exactly one line per run; they never rewrite or truncate existing lines.
- **Write-but-don't-stage.** Skills append to the working-tree file but never `git add` it — the run-log stays out of the resolution's PR diff (the line is written *after* PR creation, so it cannot be part of the resolution commits anyway).
- **Deletable and non-fatal.** The file may be deleted like any other `.gitissue/` artifact. Skills create or append to it on the next run; `/idd-doctor` degrades gracefully when it is absent, empty, or contains malformed lines (it skips unparseable lines rather than erroring).
- **Schema-light and forward-compatible.** New optional fields may be added over time; consumers ignore unknown fields and tolerate missing optional ones. There is no schema version and no migration.
