...
> constraint #6 in `references/agents/implementer.md`.
...
(`references/docs/idd-methodology.md` → *Analysis Artifacts and Durable Memory*) it must land in
...
1. **Implementer returns it.** The implementer adds a `reproduction` block to its Output
   (see `references/agents/implementer.md` → *Output*): the command, its red status, the
