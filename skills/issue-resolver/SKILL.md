...
**Per-role overrides:** apply `references/docs/agent-overrides.md` at every spawn — resolved `agents.model.<role>` / `agents.effort.<role>` for the role being spawned (`codebase-researcher`, `synthesizer`, `implementer`, `code-reviewer`, `ui-reviewer`, `fixer`). `null` (the default) passes nothing, leaving the call above as written.
