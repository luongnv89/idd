...
4. **Squash-commit binding** — check 3 proves the durable record is in the PR body; only the platform's squash-commit *message source* decides whether it reaches git history. On GitHub that is the repo's `squash_merge_commit_message` setting, and its default (`COMMIT_MESSAGES`) writes the list of commit subjects, not the PR body. **Read it; never infer it from check 3 or from the merge strategy** — that inference is the defect this check replaced (issue #295):
   ...
   ```
   That read goes through `gh api` rather than `--json` field selection because `gh repo view --json squashMergeCommitMessage` returns `Unknown JSON field` (`references/docs/platform-github.md` → *Operation catalog* → *Preflight*). Outcomes:
   - `squash_merge_commit_message == PR_BODY` → the binding holds; check 4 passes and check 3's fields are the durable record.
   - Any other value → the binding is **defeated**: report `partial`, name the observed value, and state that the record will stay on the tracker only. This check tests **B1** specifically; a repo that has declared B2 (merge-commit body) or B3 (git notes) as its binding is not defeated by a non-`PR_BODY` value, so report `n/a — repo declares {binding}` there instead of `partial`.
