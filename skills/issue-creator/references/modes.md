...
Spawn the duplicate-detector subagent once for the **pooled** medium band across all items, only when it is non-empty. Read `references/agents/duplicate-detector.md` for the full prompt.
...
Check each batch item against existing issues AND against other items in the batch, using the scoring table in `references/agents/duplicate-detector.md`.
