...
**Rate limiting:** If `gh issue create` returns a *secondary* rate limit, retry on the bounded exponential backoff of driver rule 5 in `references/docs/platform-github.md` rather than a schedule of this file's own; once that budget is exhausted, skip the item and continue with the remaining ones. *Primary* rate-limit exhaustion is not retryable — stop and print the *Rate limited* error.
