...
`references/docs/sync-conventions.md`, immediately before that write.
...
**Auto mode (`references/docs/auto-mode.md`) — never blocks.** Do not show the `Continue creating? [Y/n]` prompt. **Proceed with creation** (the interactive default), logging one `⚠` per flagged duplicate so the override is auditable:
...
In auto mode `Preview:` reads `✓ auto-approved`, not `✓ approved`: never report an approval no human gave (`references/docs/auto-mode.md`).
