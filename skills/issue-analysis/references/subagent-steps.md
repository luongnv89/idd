...
Both spawns in this file apply `references/docs/agent-overrides.md` for their role: `agents.model.codebase-researcher` / `agents.effort.codebase-researcher` for the explorer, `agents.model.synthesizer` / `agents.effort.synthesizer` for the synthesizer. A `null` value (the default) passes nothing, leaving the spawn unchanged.
