# /issue-analysis — Inline Fallback Procedure (Steps 2-7)

**Gate: read this file only when the Agent tool is unavailable.** When the Agent
tool is available, Steps 2-5 run inside the Codebase Researcher subagent and
Steps 6-7 inside the Synthesizer subagent — the delegation spec in
`references/subagent-steps.md` is all a delegated run needs, and nothing below
applies to it.

Without the Agent tool the orchestrator performs the same work itself. Execute
the procedures below in order, producing the same structured outputs the
subagents would have returned, then hand off to Step 8 as usual.

---

## Steps 2-5 — Explorer Phase (inline)
...
## Steps 6-7 — Synthesizer Phase (inline)
