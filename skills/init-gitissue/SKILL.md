...
After a successful write, when `which gh` succeeds and `gh auth status` passes, run **both** squash-merge preflight reads from `references/docs/platform-github.md` — the strategy allow-flags and the squash-commit message source. They answer different questions and neither substitutes for the other:
