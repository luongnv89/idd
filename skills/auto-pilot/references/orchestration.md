...
2. **Triage/Pick** — pick from the triage cache; the issue list is fetched and the order computed on the first iteration or a forced re-triage, not every time round the loop (or walk the explicit list)
