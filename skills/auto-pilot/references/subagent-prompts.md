...
Instructions:
1. Use the ../issue-resolver/SKILL.md skill with the `--no-run-log` flag — auto-pilot writes the single `.gitissue/runs.jsonl` line for this issue itself, so the resolver must NOT append its own (it returns telemetry instead; see the report-back fields below)
