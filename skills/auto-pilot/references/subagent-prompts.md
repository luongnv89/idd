...
5. Workspace contract depends on `parallel_lane`:
   ...
   - Block present: export `IDD_CALLER_WORKTREE=1`. The main agent created the dedicated worktree and MUST spawn you with structural `cwd` equal to `parallel_lane.worktree_path`. Run Step 0e's **Caller-managed parallel worktree validation** in full (cwd/top-level, branch, registered worktree ownership, lane/base identity, and safe resume state). Skip only the interactive offer, worktree creation, Repo Sync, and Step 0f after validation passes. Never create, fall back to, or clean up a worktree; lane ownership stays with auto-pilot.
