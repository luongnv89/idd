...
- `autopilot.triage_cache_max_age_minutes: 60` — how old `.gitissue/triage.json` may be and still be reused by *Step 1.0*'s cache gate. Age is only half the test: the gate also requires that no commit landed after the cache's own `updated` timestamp, so a cache written a minute ago is still `stale` once a merge lands. Set `0` to disable reuse and triage on every iteration, which is the pre-#258 behavior. The gate fail-safes to `stale` on any doubt, so a wrong value costs a re-triage and never a wrong pick.
