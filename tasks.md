...
- **Deliverables**:
  - [x] `skills/issue-resolver/SKILL.md`
  - [x] `skills/issue-resolver/references/error-messages.md`
  - [x] Guard checks: assignment + blocking labels → warn before proceeding
  - [x] Auto-normalization when `auto_normalize: true`
  - [x] Branch creation: `issue-N/short-description`
  - [x] Research phase: read affected files, trace dependencies (using Agent tool for parallelism)
  - [x] Plan phase: local only, approval gate support (auto / comment-and-wait)
  - [x] Execute phase: write code, write tests, atomic commits (max_commits check)
  - [x] Verify phase: run tests with timeout (`resolve.test_timeout`), check acceptance criteria
  - [x] Ship phase: `gh pr create` with "Closes #N", summary, approach, files changed
  - [x] Prompt injection boundary: issue body treated as untrusted data
  - [x] Terminal output per DESIGN.md resolve mockup (step counter [1/7])
  - [x] Static sequential progress (each step prints new line)
  - [x] Error: tests fail → abort PR, show failure details
  - [x] Error: tests timeout → abort, report timeout
  - [x] Error: branch exists → prompt: continue or fresh
...
- **Deliverables**:
  - [x] Multi-item extraction with boundary detection
  - [x] Screenshot/image input support
  - [x] Preview table before creating: `# | Type | Title | Effort`
  - [x] User can approve all, edit individual items, or cancel
  - [x] Partial failure handling: `✓ 3/5 created, ⚠ 2 failed [retry hint]`
  - [x] Rate limit respect with backoff
...
- **Deliverables**:
  - [x] Test: resolve issue end-to-end → verify branch, code, tests, PR created with "Closes #N"
  - [x] Test: resolve with assignment guard → verify warning
  - [x] Test: resolve with blocking label → verify warning
  - [x] Test: tests fail during verify → verify no PR created
  - [x] Test: branch exists → verify prompt
  - [x] Test: prompt injection in issue body → verify agent follows pipeline only
  - [x] Test: approval gate pause (mocked)
  - [x] Test: batch create from document → verify preview table + all issues created
...
- **Deliverables**:
  - [x] `skills/issue-triage/SKILL.md`
  - [x] `skills/issue-triage/references/error-messages.md`
  - [x] Fetch all open issues via `gh issue list --json` (bodies included, avoid N+1)
  - [x] Dependency detection via shared affected files and module dependencies
  - [x] Dependency table: `# | Issue | Pri | Blocks | Status`
  - [x] Topological sort for execution order
  - [x] Parallelizable issue identification
  - [x] Stale issue detection (> configured threshold)
  - [x] Circular dependency warning
  - [x] `--limit N` flag for large backlogs
  - [x] Terminal output per DESIGN.md triage mockup (table + recommendations)
  - [x] Empty state: `○ No open issues found. Nothing to triage!`
...
- **Deliverables**:
  - [x] `skills/init-gitissue/SKILL.md`
  - [x] Detect project language, framework, test runner
  - [x] Detect existing `.github/ISSUE_TEMPLATE/` templates
  - [x] Suggest defaults based on repo size and conventions
  - [x] Write `.gitissue.yml` with inline comments explaining each setting
  - [x] Report setup summary
- **Depends on**: Task 1.3
...
- **Deliverables**:
  - [x] Confidence levels: high (direct match), medium (keyword inference), low (best guess)
  - [x] Display in normalization preview: `+ Files: auth.py (high), config.py (medium)`
  - [x] Low-confidence fields marked `(needs review)` in the issue body
  - [x] Confidence displayed before applying (preview step)
...
- **Deliverables**:
  - [x] IDD methodology overview (what it is, why, how)
  - [x] Comparison with TDD, BDD, spec-driven development
  - [x] Quick-start guide in README
  - [x] Full command reference
  - [x] Configuration reference
  - [x] Agent compatibility documentation (demonstrate with 3+ agents)
...
- **Deliverables**:
  - [x] Test: triage 10+ issues → verify dependency table and execution order
  - [x] Test: triage with circular dependencies → verify warning
  - [x] Test: triage with no open issues → verify empty state
  - [x] Test: init on fresh repo → verify .gitissue.yml created
  - [x] Test: confidence scoring → verify high/medium/low displayed correctly
