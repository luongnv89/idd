...
6. **Metadata** — suggested priority, estimated effort (XS/S/M/L/XL), suggested labels, and an advisory **Suggested model:** line keyed off the effort band — see `references/model-suggestion.md`. The suggestion **always names exactly two models — one OpenAI model and one Anthropic model — joined by ` · ` (OpenAI first)**, read from the effort band's `openai` and `anthropic` entries in the cache. Never collapse it to a single model or a single provider. When `model_suggestion.enabled` is false, remove the `**Suggested model:**` line from the Metadata section entirely, matching pre-feature behaviour.
