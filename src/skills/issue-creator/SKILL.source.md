...
Scoring is deterministic and runs in `shared/scripts/gi-dup-score.py`, its GitHub call delegated to the bundled `shared/scripts/gi-gh.py` boundary and read through the shared open-issue snapshot in `shared/scripts/gi-backlog.py` (5-minute TTL; any snapshot failure falls back to a live fetch). Only the **medium-band judgement (Step 3)** is delegated to a subagent — read `shared/agents/duplicate-detector.md` for its prompt. That keeps up to 100 issue bodies out of the main agent's token budget. Every other step stays in the main agent.

### Environment check

With the Agent tool available, spawn only when Step 3 has medium candidates, applying `docs/agent-overrides.md` for the `duplicate-detector` role (`agents.model.duplicate-detector` / `agents.effort.duplicate-detector`; `null`, the default, passes nothing). Without it, report those candidates as possible duplicates rather than pretending an LLM verdict occurred.
...
trap 'exit 130' INT
trap 'exit 143' TERM
dup_output=$(python3 shared/scripts/gi-dup-score.py --snapshot < "$dup_request")
dup_status=$?
cleanup_dup_request
...
The body is the populated template, `<!-- gitissue:normalized v1 -->` at the top. After each successful create, **mandatory**: `python3 shared/scripts/gi-backlog.py --invalidate` (on failure, `rm -f .gitissue/cache/backlog-open-*.json`), so the next dedup scan sees the new issue. Print a step-by-step summary:
