...
**Per-role overrides** come from `docs/agent-overrides.md`, applied at each of the
spawns in the diagram above (`codebase-researcher`, `synthesizer`, `implementer`,
then Step 4's `code-reviewer`, `ui-reviewer` and `fixer`).
