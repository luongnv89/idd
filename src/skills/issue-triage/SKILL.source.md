...
Compute report age from the `updated` timestamp. Render the triage table in Step 8's `docs/terminal-style.md` format, under a cache header:
...
Read the open list through the shared snapshot in `shared/scripts/gi-backlog.py` (its GitHub call goes through `shared/scripts/gi-gh.py`); `/issue-creator`'s dedup reads the same file:
