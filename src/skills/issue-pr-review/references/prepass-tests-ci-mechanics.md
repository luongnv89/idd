...
After the pre-commit security scan passes (or warnings are accepted — see the scan contract in SKILL.md and the Primary Pattern in `docs/pre-commit-security.md`), commit and push:
...
When checks are still running after `review.ci_timeout`: in interactive mode, ask to wait more or proceed; in auto mode, proceed — the next cycle will re-check.
