...
| `review.ignore_ci_billing_failures: false` | `true` makes a terminal Step 5 `fail` non-blocking **at this skill's review gate only**: CI is still polled and reported, Step 6 raises no CI fixable, and the soft-pass conjunction treats the CI leg as satisfied (in strict `soft_pass: false` mode too). `ci_status` still binds as `failed@<sha40>`, the step reports `PARTIAL`, `--auto` does not merge it, and `/auto-pilot`'s merge gate re-runs its own wait and still refuses. **It does not touch `ci_leg_runnable`** — see below. GitHub exposes no API field naming a billing failure, so it ignores *any* terminal CI failure. |
...
*ran*; the ignore flag asks whether a CI *failure blocks*. Those are different
questions, and conflating them would let one config key silently skip the local
suite as well: a repo that ignores CI failures would end up with no test
evidence at all. So a `trusted` marker's `tests=` skip is decided exactly as
above whatever the ignore flag says. The safe direction is the same one: a wrong
answer here makes the local suite *run*, never skip.
