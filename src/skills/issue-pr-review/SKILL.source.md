...
Read `shared/agents/code-reviewer.md` and `shared/agents/fixer.md` for the two prompts; both spawn with the default general-purpose agent (do NOT set `subagent_type`), applying `docs/agent-overrides.md` for the `code-reviewer`, `fixer` and `ui-reviewer` roles — `agents.model.<role>` / `agents.effort.<role>`, `null` passing nothing. Pass `branch_name`, `base_branch`, `pr_context` (title + body), `diff_command` (`gh pr diff {N}`), and `review.confidence_threshold` (default 80) as the minimum finding confidence; ui-reviewer keeps its 75 floor.
