...
`--auto` is set automatically by `/auto-pilot`. In auto mode export `IDD_AUTO_MODE=1` before any shell snippet that consults it — the pre-commit security scan reads it to switch from prompt-on-warning to log-and-continue (`docs/pre-commit-security.md`). `--no-merge` suppresses auto-merge even under `--auto`, for when another agent owns the merge step.
