...
- **Linked-issue `Effort` band** — when the PR links an issue (`Closes #N`), fetch
  its `## Metadata` `Effort` (`python3 shared/scripts/gi-issue.py {N} --fields body`,
...
Also fetch the linked issue for acceptance-criteria verification: `python3 shared/scripts/gi-issue.py {linked_issue} --fields number,title,body,labels`, reading `.issue`. Step 1's depth gate already read this issue, so the cache usually answers without a second network call. Exit 3 is a stop; exit 4 or no `python3` degrades to `gh issue view {linked_issue} --json number,title,body,labels`.
