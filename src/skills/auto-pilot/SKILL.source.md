...
9. **Confirm push/merge permission** (driver `docs/platform-github.md` ~22-23) with `gh repo view --json viewerPermission`. `ADMIN`, `MAINTAIN` or `WRITE` can push and merge — proceed. `READ`, `TRIAGE` or `NONE` cannot merge: rather than fail, **downgrade to no-merge mode** — print `⚠ Insufficient merge permission — running in no-merge mode` from `references/error-messages.md`, run the full triage/resolve/review loop, and skip Phase 5, leaving every PR open for a maintainer. Leaving PRs open beats failing.
